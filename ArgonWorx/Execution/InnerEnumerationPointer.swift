...
public class InnerEnumerationPointer: InnerPointer
    {
    public class func allocate(in segment: ManagedSegment) -> InnerEnumerationPointer
        {
        let address = segment.allocateObject(sizeInBytes: Self.kEnumerationSizeInBytes)
        let pointer = InnerEnumerationPointer(address: address)
        pointer.assignSystemSlots(from: ArgonModule.argonModule.enumeration)
        return(pointer)
        }
