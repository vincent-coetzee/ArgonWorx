...
public class InnerInstructionArrayPointer:InnerArrayPointer
    {
    public static let samples = InnerInstructionArrayPointer.allocate(arraySize: 10*10, in: ManagedSegment.shared)
        .append(.make,operand1: .address(ArgonModule.argonModule.array.memoryAddress),operand2: .integer(1024),result: .register(.r0))
        .append(.store,operand1:.register(.r1),result: .register(.fp))
