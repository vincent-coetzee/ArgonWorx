...
public class InnerStringKeyDictionaryPointer: InnerDictionaryPointer
    {
    public class func allocate(size:Int,in segment:ManagedSegment) -> InnerStringKeyDictionaryPointer
        {
        var prime = size * 5 / 2
        while !prime.isPrime
            {
        ...
        pointer.prime = prime
        pointer.assignSystemSlots(from: ArgonModule.argonModule.dictionary)
    ...
    public subscript(_ key:String) -> Word?
        ...
        set
            ...
            if bucketAddress == 0
                {
                let keyAddress = InnerStringPointer.allocateString(key, in: ManagedSegment.shared)
                let bucket = InnerDictionaryBucketPointer.allocate(key: keyAddress.address, value: (newValue ?? 0), next: 0, in: ManagedSegment.shared)
