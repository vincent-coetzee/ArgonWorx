...
public class InnerEnumerationCasePointer: InnerPointer
    {
    public class func allocate(in segment: ManagedSegment) -> InnerEnumerationCasePointer
        {
        let address = segment.allocateObject(sizeInBytes: Self.kEnumerationCaseSizeInBytes)
        let pointer = InnerEnumerationCasePointer(address: address)
        pointer.assignSystemSlots(from: ArgonModule.argonModule.enumerationCase)
        return(pointer)
        }
