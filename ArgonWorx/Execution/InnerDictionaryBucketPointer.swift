...
public class InnerDictionaryBucketPointer: InnerPointer
    {
    public class func allocate(key:Word, value: Word,next: Word,in segment: ManagedSegment) -> InnerDictionaryBucketPointer
        {
        let address = segment.allocateObject(sizeInBytes: Self.kDictionaryBucketSizeInBytes)
